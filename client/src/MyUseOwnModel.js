...
function ComputePrediction(){
    ...
    useEffect(() => {
      const retriveAvailableModels = async () => {
          API.availableOwnModels().then(res => setAvailableModels(res) )
      };
    ...
    const handleSubmit = async (e) => { //EXTRACT THE POWER
      ...
      const handleNewModel = async (e) => {
        ...
        const files = document.getElementById("newModel");
        const formData = new FormData();
        for (let i = 0; i < files.files.length; i++) {
            formData.append("files", files.files[i]);
        }
        const response = fetch("http://localhost:5000/upload_newModel", {
            method: 'POST',
            body: formData
        })
      ...
      const computePrediction = async () => {
        ...
        if(ready && chosenModels.length > 0){
          let result = await API.getPrediction(signal, chosenModels, true);
    ...
    return (
        ...
        <div>
          <Row>
        <h3>Upload the new trained model</h3>
        <input type="file" id="newModel" multiple onChange={handleNewModel}></input>
      </Row>
      <hr></hr>
      <Row>
